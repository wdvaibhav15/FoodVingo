...
export const getMyShop = async (req, res) => {
  try {
    ...
    if (!shop) {
      return res.status(200).json({ message: "No shop found", shop: null });
    }

    // Safety fallback: if shop.items is empty, fetch all items matching the shop ID directly
